...
def parse_args():
    ...
    parser.add_argument(
        '--dataset', help="sim10k", default=None, type=str)
    parser.add_argument(
    ...
    parser.add_argument(
        '--imgdir', help="directory to parse for subdirs of MUNIT modes",
        type=str)
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
...
def parseXmlFiles(xml_path, img_subdir):
    for f in sorted(os.listdir(xml_path)):
        ...
        for elem in root:
            ...
            if elem.tag == 'filename':
                file_name = elem.text
                file_name = os.path.join(img_subdir, f.split('.xml')[0]+'.jpg')
            ...
            for subelem in elem:
                ...
                if bndbox['xmin'] is not None:
                    ...
                    # MUNIT runs on 512 pixels on the shorter side (which is the height of sim10k images)
                    for i in range(4):
                        bbox[i] *= 512 / 1052
...
if __name__ == '__main__':
    ...
    if args.dataset != "sim10k":
        raise Exception('ataset not supported: {}'.format(args.dataset))
    json_file = 'car_instances.json'
    addCatItem('car')
    img_subdirs = [
    ...
    for img_subdir in img_subdirs:
        print(f"parsing {img_subdir}")
        parseXmlFiles(args.datadir, img_subdir)
    json.dump(coco, open(os.path.join(args.outdir, json_file), 'w'))
