...
~[Pretrained model](https://polybox.ethz.ch/index.php/s/OgkNFJHVkEscTO0) with image+instance+consistency domain adaptation on Resnet-50 bakcbone for Cityscapes->Foggy Cityscapes task is provided. For those who might be interested, the corresponding training log could be checked at [here](logs/city2foggy_r50_consistency_log.txt).~

As the link above is not valid anymore, a newly pretrained model and the training log under the same setting could be found on [Google drive](https://drive.google.com/drive/folders/1d5jnKE1_Mu2skgHCiGv0PD9-ixxr2rvF?usp=sharing). Due to the change of running environment and the random number, the number would be slightly different compared to the previous one.
